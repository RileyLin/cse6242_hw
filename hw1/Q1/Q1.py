...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        # Remove commas from name to prevent CSV issues
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        # For undirected graph, ensure consistent edge ordering to avoid duplicates
    ...
    def max_degree_nodes(self) -> dict:
        ...
        # Return all nodes with maximum degree
        return {node_id: degree for node_id, degree in degree_count.items() if degree == max_degree}
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id: str, limit: int = None, exclude_ids: list[int] = None) -> list:
        ...
        try:
            ...
            if response.status != 200:
                print(f"API request failed with status {response.status} for movie {movie_id}")
                return []
            ...
            # Filter by limit first (order 0 to limit-1)
            if limit is not None:
                cast = [member for member in cast if member.get('order', float('inf')) < limit]
            
            # Then exclude specified IDs
            if exclude_ids:
                exclude_ids_set = set(exclude_ids)
                cast = [member for member in cast if member.get('id') not in exclude_ids_set]
            
            return cast
            
        except Exception as e:
            print(f"Error fetching movie cast for movie {movie_id}: {e}")
            return []
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        try:
            ...
            if response.status != 200:
                print(f"API request failed with status {response.status} for person {person_id}")
                return []
                
            credits_data = json.loads(data.decode('utf-8'))
            ...
            print(f"Error fetching movie credits for person {person_id}: {e}")
            return []
...
def build_co_actor_network(api_key: str) -> Graph:
    """
    Build the co-actor network starting with Laurence Fishburne for 1999 movies
    """
    graph = Graph()
    tmdb_api_utils = TMDBAPIUtils(api_key=api_key)
    
    # Initialize with Laurence Fishburne
    laurence_id = '2975'
    graph.add_node(id=laurence_id, name='Laurence Fishburne')
    
    print("Building base graph...")
    
    # BUILD BASE GRAPH: Get Laurence Fishburne's 1999 movies and co-actors
    laurence_movies = tmdb_api_utils.get_movie_credits_for_person(
        person_id=laurence_id, 
        start_date='1999-01-01', 
        end_date='1999-12-31'
    )
    
    new_nodes_added = set()  # Track new nodes for the first iteration
    
    for movie in laurence_movies:
        movie_id = str(movie['id'])
        print(f"Processing movie: {movie['title']} ({movie_id})")
        
        # Get top 5 cast members (order 0-4), excluding Laurence Fishburne
        cast_members = tmdb_api_utils.get_movie_cast(
            movie_id=movie_id,
            limit=5,
            exclude_ids=[int(laurence_id)]
        )
        ...
        for cast_member in cast_members:
            ...
            # Add the co-actor as a node
            graph.add_node(id=actor_id, name=actor_name)
            new_nodes_added.add(actor_id)
            
            # Add edge between Laurence Fishburne and co-actor
            graph.add_edge(laurence_id, actor_id)
        
        # Rate limiting
        time.sleep(0.25)  # 4 requests per second
    
    print(f"Base graph complete. Added {len(new_nodes_added)} co-actors.")
    
    # EXPAND GRAPH: Do 2 iterations
    for iteration in range(2):
        print(f"Starting iteration {iteration + 1}...")
        
        # Use nodes from previous iteration
        nodes_to_process = list(new_nodes_added)
        new_nodes_added = set()
        
        for actor_id in nodes_to_process:
            print(f"Processing actor ID: {actor_id}")
            
            # Get this actor's 1999 movies
            actor_movies = tmdb_api_utils.get_movie_credits_for_person(
                person_id=actor_id,
                start_date='1999-01-01',
                end_date='1999-12-31'
            )
            ...
            for movie in actor_movies:
                ...
                # Get top 5 cast members for this movie
                cast_members = tmdb_api_utils.get_movie_cast(
                    movie_id=movie_id,
                    limit=5
                )
                ...
                for cast_member in cast_members:
                    ...
                    co_actor_name = cast_member.get('name', 'Unknown')
                    
                    # Check if node already exists
                    existing_node_ids = {node[0] for node in graph.nodes}
                    if co_actor_id not in existing_node_ids:
                        graph.add_node(id=co_actor_id, name=co_actor_name)
                        new_nodes_added.add(co_actor_id)
                    
                    # Add edge if it doesn't exist (add_edge handles duplicates)
                    graph.add_edge(actor_id, co_actor_id)
                
                # Rate limiting
                time.sleep(0.25)
        
        print(f"Iteration {iteration + 1} complete. Added {len(new_nodes_added)} new actors.")
    
    print(f"Final graph: {graph.total_nodes()} nodes, {graph.total_edges()} edges")
    return graph


if __name__ == "__main__":
    api_key = '716696e9e088d5b5629751a311a943f7'
    ...
    try:
        ...
        print(f"Max degree nodes: {graph.max_degree_nodes()}")
        
    except Exception as e:
        print(f"Error building graph: {e}")
        # Fallback: create minimal graph for testing
        graph = Graph()
        graph.add_node(id='2975', name='Laurence Fishburne')
        graph.write_edges_file()
        graph.write_nodes_file()
    
    # If you have already built & written out your graph, you can read it back:
