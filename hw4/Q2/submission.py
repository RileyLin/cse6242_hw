...
class Utility(object):
    ...
    def best_split(self, X, y):
       ...
       'y_right': [0, 0, 1]
        ...
        # For random forest: randomly select sqrt(num_features) attributes
        num_features_to_try = int(np.sqrt(num_features))
        if num_features_to_try < 1:
            num_features_to_try = 1
        
        # Randomly select features without replacement
        feature_indices = random.sample(range(num_features), min(num_features_to_try, num_features))
        ...
        for feature_idx in feature_indices:
            ...
            for val in unique_values:
                X_l, X_r, y_l, y_r = self.partition_classes(X, y, feature_idx, val)
                ...
                current_y = [y_l, y_r]
                gain = self.information_gain(y, current_y)
                
                # Update best split if this is better
                if gain > best_info_gain:
                    best_info_gain = gain
                    split_attribute = feature_idx
                    split_val = val
                    X_left = X_l
                    X_right = X_r
                    y_left = y_l
        ...
        return {'split_attribute': split_attribute, 'split_val': split_val, 
...
class DecisionTree(object):
    ...
    def learn(self, X, y, par_node = {}, depth=0):
        ...
        # If max depth reached, create a leaf node with majority class
        ...
        # If no valid split found (info_gain = -1 or 0), create leaf node
        if split_info['info_gain'] <= 0:
            counts = {}
            for label in y:
                counts[label] = counts.get(label, 0) + 1
            majority_class = max(counts, key=counts.get)
        ...
        node['left'] = self.learn(split_info['X_left'], split_info['y_left'], {}, depth + 1)
        node['right'] = self.learn(split_info['X_right'], split_info['y_right'], {}, depth + 1)
        
        # Store the tree if this is the root
        if depth == 0:
            self.tree = node
...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        for _ in range(n):
            ...
            idx = random.randint(0, len(XX) - 1)
            # XX contains features + label in last column
            sample.append(XX[idx][:-1])  # All columns except last
            labels.append(XX[idx][-1])   # Last column is label
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            # Convert votes to int to avoid bincount warning
            ...
            if len(counts) == 0:
                ...
                ### Implement your code here
                #############################################
                # If the record is in all bootstrap samples (not OOB for any tree),
                # use all trees to vote
                votes_all = []
                for i in range(len(self.decision_trees)):
                    vote = self.decision_trees[i].classify(record)
                    votes_all.append(int(vote))
                counts = np.bincount(votes_all)
                #############################################
